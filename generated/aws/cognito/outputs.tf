# 사용자 풀 클라이언트 출력
output "aws_cognito_user_pool_client_tfer--250822_test_783nooe1lkidr7l7ac91di2q4i_id" {
  value = "${aws_cognito_user_pool_client.tfer--250822_test_783nooe1lkidr7l7ac91di2q4i.id}"
}

output "aws_cognito_user_pool_client_tfer--My-0020-SPA-0020-app-0020---0020-ak0udl_62i5q9et3hjepf7kt8ujs5gkhu_id" {
  value = "${aws_cognito_user_pool_client.tfer--My-0020-SPA-0020-app-0020---0020-ak0udl_62i5q9et3hjepf7kt8ujs5gkhu.id}"
}

output "aws_cognito_user_pool_client_tfer--dugout-main_3a58ij7t5e5va7qjkfdmjsghq_id" {
  value = "${aws_cognito_user_pool_client.tfer--dugout-main_3a58ij7t5e5va7qjkfdmjsghq.id}"
}

output "aws_cognito_user_pool_client_tfer--dugout-users-public_client_28m777op4t7t3ufms2f3197bp4_id" {
  value = "${aws_cognito_user_pool_client.tfer--dugout-users-public_client_28m777op4t7t3ufms2f3197bp4.id}"
}

output "aws_cognito_user_pool_client_tfer--dugout-users_45e0a4p1ivur4oas8sea467887_id" {
  value = "${aws_cognito_user_pool_client.tfer--dugout-users_45e0a4p1ivur4oas8sea467887.id}"
}

# 사용자 풀 출력 (lrmgzt만 유지)
output "aws_cognito_user_pool_tfer--User-0020-pool-0020---0020-lrmgzt_ap-northeast-2_KYxl8SX2a_id" {
  value = "${aws_cognito_user_pool.tfer--User-0020-pool-0020---0020-lrmgzt_ap-northeast-2_KYxl8SX2a.id}"
}

output "aws_cognito_user_pool_tfer--User-0020-pool-0020---0020-lrmgzt_ap-northeast-2_KYxl8SX2a_arn" {
  value = "${aws_cognito_user_pool.tfer--User-0020-pool-0020---0020-lrmgzt_ap-northeast-2_KYxl8SX2a.arn}"
}