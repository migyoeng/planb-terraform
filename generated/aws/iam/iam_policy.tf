...
data "terraform_remote_state" "cognito" {
  backend = "local"
  config = {
    path = "../cognito/terraform.tfstate"
  }
}

# Secrets Manager Remote State 데이터 소스
data "terraform_remote_state" "secretsmanager" {
  backend = "local"
  config = {
    path = "../secretsmanager/terraform.tfstate"
  }
}

# Cognito 사용자 삭제 정책
resource "aws_iam_policy" "DugoutCognitoDeleteUser" {
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Resource = "arn:aws:cognito-idp:ap-northeast-2:${data.aws_caller_identity.current.account_id}:userpool/${data.terraform_remote_state.cognito.outputs.aws_cognito_user_pool_tfer--User-0020-pool-0020---0020-lrmgzt_ap-northeast-2_KYxl8SX2a_id}"
      }
    ]
  })
}

# ECS Secrets Manager 정책
resource "aws_iam_policy" "ECSGetSecretsManager" {
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Resource = [
          data.terraform_remote_state.secretsmanager.outputs.aws_secretsmanager_secret_tfer--prod-002F-cognito-config_arn,
          data.terraform_remote_state.secretsmanager.outputs.aws_secretsmanager_secret_tfer--prod-002F-user-002F-mysql-vm_arn,
          data.terraform_remote_state.secretsmanager.outputs.aws_secretsmanager_secret_tfer--prod-002F-board-002F-mysql_arn,
          data.terraform_remote_state.secretsmanager.outputs.aws_secretsmanager_secret_tfer--prod-002F-news-002F-mysql_arn,
          data.terraform_remote_state.secretsmanager.outputs.aws_secretsmanager_secret_tfer--prod-002F-event-002F-mysql_arn
        ]
      }
    ]
  })
...
# ECS ECR Pull Image 정책
resource "aws_iam_policy" "ECSPullECRImage" {
  name        = "ECSPullECRImage"
  description = "ECS ECR Image Pull"
  path        = "/"
