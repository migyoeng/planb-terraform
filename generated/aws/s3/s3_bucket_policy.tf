...
resource "aws_s3_bucket_policy" "tfer--dugout-frontend-test" {
  bucket = aws_s3_bucket.tfer--dugout-frontend-test.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Action   = "s3:GetObject"
        Resource = "${aws_s3_bucket.tfer--dugout-frontend-test.arn}/*"
        Condition = {
          StringEquals = {
            "AWS:SourceArn" = "arn:aws:cloudfront::${data.aws_caller_identity.current.account_id}:distribution/${data.terraform_remote_state.cloudfront.outputs.aws_cloudfront_distribution_tfer--E1NQOA9YMS3RQ6_id}"
          }
        }
      }
    ]
